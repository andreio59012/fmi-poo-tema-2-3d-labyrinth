...
struct vec2i {
    ...
    vec2i(const vec2& src);

    vec2i& operator+=(const vec2i& a) { x += a.x; y += a.y; return *this; }
    vec2i& operator-=(const vec2i& a) { x -= a.x; y -= a.y; return *this; }
    vec2i& operator*=(const float a) { x = int(x * a); y = int(y * a); return *this; }
    vec2i& operator*=(const vec2i& a) { x *= a.x; y *= a.y; return *this; }
    vec2i& operator/=(const float a) { x = int(x / a); y = int(y / a); return *this; }
    vec2i& operator/=(const vec2i& a) { x /= a.x; y /= a.y; return *this; }
};

inline vec2::vec2(const vec2i& src) : x(src.x), y(src.y) {}
inline vec2i::vec2i(const vec2& src) : x((int)src.x), y((int)src.y) {}
...
inline int clamp(const int a, const int a_min, const int a_max) {
